...
//
bool test_set_SRP0(Print& sio, uint32_t qe_bit, bool use_16_bit_sr1, bool non_volatile, [[maybe_unused]] bool was_preset) {
  bool ok = false;
...
// BergMicro, XMC.
//
bool test_clear_SR1([[maybe_unused]] Print& sio, uint32_t qe_bit, bool use_16_bit_sr1,
...
//
bool test_set_QE(Print& sio, uint32_t qe_bit, bool use_16_bit_sr1, bool non_volatile, bool was_preset) {
  spi0_flash_write_disable();
  bool qe = false;
...
//
bool testFlashWrite([[maybe_unused]] Print& sio, uint32_t qe_bit, bool use_16_bit_sr1, bool non_volatile) {
  bool test = false;
  spi0_flash_write_disable();
...
//
bool testOutputGPIO10(Print& sio, uint32_t qe_bit, bool use_16_bit_sr1, bool non_volatile, bool was_preset) {
  sio.PRINTF("\nRunning verification test for pin function /WP disable\n");
  sio.PRINTF("and confirm pin is ignored on flash writes\n");
  if (9u != qe_bit && 6u != qe_bit) {
    sio.PRINTF("* QE/S%u bit field specification undefined should be either S6 or S9\n", qe_bit);
    return false;
  }
  bool has_QE = test_set_QE(sio, qe_bit, use_16_bit_sr1, non_volatile, was_preset);
  uint32_t good_count = 0;

  sio.PRINTF("  Test Write: QE/S%u=%c, GPIO10 as OUTPUT\n", qe_bit, (has_QE) ? '1' : '0');
  pinMode(10, OUTPUT);      // was /WP
  digitalWrite(10, HIGH);
  bool success = testFlashWrite(sio, qe_bit, use_16_bit_sr1, non_volatile);
  if (success) good_count++;
  // Expect success regardless of QE
  bool pass = success; //(success && has_QE) || (!success && !has_QE);
  sio.PRINTF("%c Test Write: With /WP set HIGH write %s\n", (pass) ? ' ' : '*', (success) ? "succeeded" : "failed.");

  digitalWrite(10, LOW);
  success = testFlashWrite(sio, qe_bit, use_16_bit_sr1, non_volatile);
  if (success && has_QE) good_count++;
  // Expect success if QE=1, failure otherwise
  pass = (success && has_QE) || (!success && !has_QE);
  sio.PRINTF("%c Test Write: With /WP set LOW write %s\n", (pass) ? ' ' : '*', (success) ? "succeeded" : "failed.");
  //D if (!pass) printAllSRs(sio);
  if (!success && has_QE) sio.PRINTF("  Test Write: QE does not disable pin function /WP or QE/S%u is the wrong bit.\n", qe_bit);

  if (2 == good_count) {
    sio.PRINTF("  Test Write: Pass - confirmed QE disabled pin function /WP.\n");
    return true;
  }
  sio.PRINTF("  Test Write: Fail - unexpected results.\n");
  return false;
}

void testOutputGPIO9(Print& sio, uint32_t qe_bit, bool use_16_bit_sr1, bool non_volatile, bool was_preset) {
  ...
  bool has_QE = test_set_QE(sio, qe_bit, use_16_bit_sr1, non_volatile, was_preset);
  sio.PRINTF("  Verify /HOLD is disabled by Status Register QE/S%u=%u\n", qe_bit, (has_QE) ? 1u : 0);
  sio.PRINTF("  If changing GPIO9 to OUTPUT and setting LOW crashes, it failed.\n");
  pinMode(9u, OUTPUT);
  digitalWrite(9u, LOW);
  if (has_QE) {
    sio.PRINTF("  passed - QE/S%u bit setting worked.\n", qe_bit); // No WDT Reset - then it passed.
  } else {
    sio.PRINTF("* Ambiguous results. QE=0 and we did not crash. Flash may not support /HOLD.\n");
  }
}

void testInput_GPIO9_GPIO10(Print& sio, uint32_t qe_bit, bool use_16_bit_sr1, bool non_volatile, bool was_preset) {
  sio.PRINTF("\nRunning GPIO9 and GPIO10 INPUT test\n");
  sio.PRINTF("Test GPIO9 and GPIO10 by reading from the pins as INPUT and print result.\n");
  if (9u != qe_bit && 6u != qe_bit) {
    sio.PRINTF("* QE/S%u bit field specification undefined should be either S6 or S9\n", qe_bit);
    return;
  }
  bool has_QE = test_set_QE(sio, qe_bit, use_16_bit_sr1, non_volatile, was_preset);
  sio.PRINTF("  Test: QE/S%u=%u, GPIO pins 9 and 10 as INPUT\n", qe_bit, (has_QE) ? 1u : 0);
  pinMode(9, INPUT);
  pinMode(10, INPUT);
  uint32_t pin9 = digitalRead(9);
  sio.PRINTF("  digitalRead result: GPIO_9(%u) and GPIO_10(%u)\n", pin9, digitalRead(10));

  if (0 == pin9){
    if (has_QE) {
      sio.PRINTF("  Passed - no crash\n"); // No WDT Reset - then it passed.
    } else {
      sio.PRINTF("* Ambiguous results. QE=0 and we did not crash. Flash may not support /HOLD.\n");
    }
  }
